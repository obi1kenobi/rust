...
// By-value outer tuple struct requires `InnerTuple<T>` to be Sized, implying `T: Sized`.
...
//@ has "$.index[?(@.name=='takes_outer_tuple_value')].inner.function.generics.params[0].kind.type.implied_bounds[?(@.trait_bound.trait.path=='Sized' && @.trait_bound.modifier=='none')]"
//@ !has "$.index[?(@.name=='takes_outer_tuple_value')].inner.function.generics.params[0].kind.type.implied_bounds[?(@.trait_bound.modifier=='maybe')]"
pub fn takes_outer_tuple_value<T: ?Sized>(value: OuterTuple<InnerTuple<T>>) {
    let _ = value;
}

// By-value outer named struct requires `InnerNamed<T>` to be Sized, implying `T: Sized`.
...
//@ has "$.index[?(@.name=='takes_outer_named_value')].inner.function.generics.params[0].kind.type.implied_bounds[?(@.trait_bound.trait.path=='Sized' && @.trait_bound.modifier=='none')]"
//@ !has "$.index[?(@.name=='takes_outer_named_value')].inner.function.generics.params[0].kind.type.implied_bounds[?(@.trait_bound.modifier=='maybe')]"

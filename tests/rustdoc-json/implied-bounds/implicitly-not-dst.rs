...
// By-value tuple structs must be Sized, so `T: ?Sized` still implies `T: Sized`.
...
//@ has "$.index[?(@.name=='takes_tuple_struct_value')].inner.function.generics.params[0].kind.type.implied_bounds[?(@.trait_bound.trait.path=='Sized' && @.trait_bound.modifier=='none')]"
...
// By-value named-field structs must be Sized, so `T: ?Sized` still implies `T: Sized`.
...
//@ has "$.index[?(@.name=='takes_named_struct_value')].inner.function.generics.params[0].kind.type.implied_bounds[?(@.trait_bound.trait.path=='Sized' && @.trait_bound.modifier=='none')]"
